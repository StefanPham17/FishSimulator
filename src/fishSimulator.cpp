...
void FishSimulator::load_textures() {
...
void FishSimulator::load_shaders() {
...
FishSimulator::FishSimulator(std::string project_root, Screen *screen)
...
void FishSimulator::loadCloth(Cloth *cloth) { this->cloth = cloth; }

void FishSimulator::loadClothParameters(ClothParameters *cp) { this->cp = cp; }

void FishSimulator::loadCollisionObjects(vector<CollisionObject *> *objects) { this->collision_objects = objects; }

/**
 * Initializes the cloth simulation and spawns a new thread to separate
 * rendering from simulation.
 */
void FishSimulator::init() {
  ...
  for (auto &f : cloth->fishes) {
    avg_pm_position += f.position / cloth->fishes.size();
  }
  ...
  ground = new Plane(Vector3D(0, -2, 0), Vector3D(0, 1, 0), 1);
}

bool FishSimulator::isAlive() { return is_alive; }

void drawBox(GLShader &shader, float height, float width) {
  MatrixXf positions(4, 24);
  // Creating the Box
  positions.col(0) <<  width,  height,  width, 1.0;
  positions.col(1) << -width,  height,  width, 1.0;

  positions.col(2) <<  width,  height,  width, 1.0;
  positions.col(3) <<  width, -height,  width, 1.0;

  positions.col(4) <<  width,  height,  width, 1.0;
  positions.col(5) <<  width,  height, -width, 1.0;
  //
  positions.col(6) << -width, -height,  width, 1.0;
  positions.col(7) << -width, -height, -width, 1.0;

  positions.col(8) << -width, -height,  width, 1.0;
  positions.col(9) << -width,  height,  width, 1.0;

  positions.col(10) << -width, -height,  width, 1.0;
  positions.col(11) <<  width, -height,  width, 1.0;
  //
  positions.col(12) << -width,  height, -width, 1.0;
  positions.col(13) << -width,  height,  width, 1.0;

  positions.col(14) << -width,  height, -width, 1.0;
  positions.col(15) << -width, -height, -width, 1.0;

  positions.col(16) << -width,  height, -width, 1.0;
  positions.col(17) <<  width,  height, -width, 1.0;
  //
  positions.col(18) <<  width, -height, -width, 1.0;
  positions.col(19) <<  width, -height,  width, 1.0;

  positions.col(20) <<  width, -height, -width, 1.0;
  positions.col(21) <<  width,  height, -width, 1.0;

  positions.col(22) <<  width, -height, -width, 1.0;
  positions.col(23) << -width, -height, -width, 1.0;

  shader.uploadAttrib("in_position", positions, false);
  shader.drawArray(GL_LINES, 0, 24);
}

void FishSimulator::drawContents() {
  glEnable(GL_DEPTH_TEST);
  ...
  if (!is_paused) {
    ...
    for (int i = 0; i < simulation_steps; i++) {
      cloth->simulate(frames_per_sec, simulation_steps, cp, external_accelerations, 
                      collision_objects, alignment, separation, cohesion, entities,
                      ground, wander, boundary, collision, hunt, flee);
    }
  }
  ...
  case PHONG:
    ...
    shader.setUniform("u_light_pos", Vector3f(0.5, 2, 0.5), false);
    shader.setUniform("u_light_intensity", Vector3f(6, 12, 12), false);
  ...
  /* Sand */
  UserShader& active_sand_shader = shaders[8];

  GLShader &sand_shader = *active_sand_shader.nanogui_shader;
  sand_shader.bind();
  sand_shader.setUniform("u_model", model);
  sand_shader.setUniform("u_view_projection", viewProjection);

  ground->render(sand_shader);

  /* Pillars */

  UserShader& active_pillar_shader = shaders[7];

  GLShader &pillar_shader = *active_pillar_shader.nanogui_shader;
  pillar_shader.bind();
  ...
  for (CollisionObject *co : *collision_objects) {
    co->render(pillar_shader);
  }
}

void FishSimulator::drawWireframe(GLShader &shader) {
  for (Fish f: cloth->fishes) {
    f.render(shader);
  }
  for (Predator p: cloth->predators) {
    p.render(shader);
  }
}

void FishSimulator::drawNormals(GLShader &shader) {
  for (Fish f: cloth->fishes) {
    f.render(shader);
  }
  for (Predator p: cloth->predators) {
...
void FishSimulator::drawPhong(GLShader &shader) {
  for (Fish f: cloth->fishes) {
    f.render(shader);
  }
  shader.setUniform("u_light_intensity", Vector3f(16, 4, 4), false);
  for (Predator p: cloth->predators) {
    p.render(shader);
  }
}
...
void FishSimulator::resetCamera() { camera.copy_placement(canonicalCamera); }

Matrix4f FishSimulator::getProjectionMatrix() {
...
Matrix4f FishSimulator::getViewMatrix() {
  Matrix4f lookAt;
  Matrix3f R;
...
bool FishSimulator::cursorPosCallbackEvent(double x, double y) {
...
bool FishSimulator::mouseButtonCallbackEvent(int button, int action,
                                              int modifiers) {
  switch (action) {
  case GLFW_PRESS:
    switch (button) {
...
void FishSimulator::mouseMoved(double x, double y) { y = screen_h - y; }

void FishSimulator::mouseLeftDragged(double x, double y) {
  float dx = x - mouse_x;
  float dy = y - mouse_y;
...
void FishSimulator::mouseRightDragged(double x, double y) {
  camera.move_by(mouse_x - x, y - mouse_y, canonical_view_distance);
}

bool FishSimulator::keyCallbackEvent(int key, int scancode, int action,
                                      int mods) {
...
bool FishSimulator::dropCallbackEvent(int count, const char **filenames) {
  return true;
}

bool FishSimulator::scrollCallbackEvent(double x, double y) {
  camera.move_forward(y * scroll_rate);
  return true;
}

bool FishSimulator::resizeCallbackEvent(int width, int height) {
  screen_w = width;
  screen_h = height;
...
void FishSimulator::initGUI(Screen *screen) {
  Window *window;
  ...
  new Label(window, "Fish Behavior", "sans-bold");

  {
    ...
    new Label(panel, "Fish :", "sans-bold");

    IntBox<int> *fb = new IntBox<int>(panel);
    fb->setEditable(true);
    fb->setFixedSize(Vector2i(100, 20));
    fb->setFontSize(14);
    fb->setValue(entities.x);
    fb->setSpinnable(true);
    fb->setCallback([this](int value) { entities.x = value; });

    new Label(panel, "Predators :", "sans-bold");

    fb = new IntBox<int>(panel);
    fb->setEditable(true);
    fb->setFixedSize(Vector2i(100, 20));
    fb->setFontSize(14);
    fb->setValue(entities.y);
    fb->setSpinnable(true);
    fb->setCallback([this](int value) { entities.y = value; });
  }

   new Label(window, "Fish Size", "sans-bold");

  {
    ...
    Slider *slider = new Slider(panel);
    slider->setValue(entities.z);
    slider->setFixedWidth(105);

    TextBox *percentage = new TextBox(panel);
    percentage->setFixedWidth(75);
    percentage->setValue(to_string(entities.z));
    percentage->setFontSize(14);
...

  {
    ...
    new Label(panel, "Alignment Radius :", "sans-bold");

    FloatBox<double> *fb = new FloatBox<double>(panel);
    fb->setEditable(true);
    fb->setFixedSize(Vector2i(100, 20));
    fb->setFontSize(14);
    fb->setValue(alignment.x);
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { alignment.x = value; });

    new Label(panel, "Alignment Weight :", "sans-bold");

    fb = new FloatBox<double>(panel);
    fb->setEditable(true);
    fb->setFixedSize(Vector2i(100, 20));
    fb->setFontSize(14);
    fb->setValue(alignment.y);
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { alignment.y = value; });

    new Label(panel, "Separation Radius :", "sans-bold");

    fb = new FloatBox<double>(panel);
    fb->setEditable(true);
    fb->setFixedSize(Vector2i(100, 20));
    fb->setFontSize(14);
    fb->setValue(separation.x);
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { separation.x = value; });

    new Label(panel, "Separation Weight :", "sans-bold");

    fb = new FloatBox<double>(panel);
    fb->setEditable(true);
    fb->setFixedSize(Vector2i(100, 20));
    fb->setFontSize(14);
    fb->setValue(separation.y);
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { separation.y = value; });

    new Label(panel, "Cohesion Radius :", "sans-bold");

    fb = new FloatBox<double>(panel);
    fb->setEditable(true);
    fb->setFixedSize(Vector2i(100, 20));
    fb->setFontSize(14);
    fb->setValue(cohesion.x);
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { cohesion.x = value; });

    new Label(panel, "Cohesion Weight :", "sans-bold");

    fb = new FloatBox<double>(panel);
    fb->setEditable(true);
    fb->setFixedSize(Vector2i(100, 20));
    fb->setFontSize(14);
    fb->setValue(cohesion.y);
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { cohesion.y = value; });

    new Label(panel, "Wander Weight :", "sans-bold");
    ...
    fb->setValue(wander);
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { wander = value; });

    new Label(panel, "Boundary Weight :", "sans-bold");

    fb = new FloatBox<double>(panel);
    fb->setEditable(true);
    fb->setFixedSize(Vector2i(100, 20));
    fb->setFontSize(14);
    fb->setValue(boundary);
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { boundary = value; });

    new Label(panel, "Collision Weight :", "sans-bold");

    fb = new FloatBox<double>(panel);
    fb->setEditable(true);
    fb->setFixedSize(Vector2i(100, 20));
    fb->setFontSize(14);
    fb->setValue(collision);
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { collision = value; });

    new Label(panel, "Hunt Weight :", "sans-bold");
    ...
    fb->setValue(hunt);
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { hunt = value; });

    new Label(panel, "Flee Weight :", "sans-bold");
    ...
    fb->setValue(flee);
    fb->setSpinnable(true);
    fb->setCallback([this](float value) { flee = value; });
  }
