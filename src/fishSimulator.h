...
class FishSimulator {
public:
  FishSimulator(std::string project_root, Screen *screen);
  ~FishSimulator();
...
private:
  ...
  CGL::Vector3D alignment = CGL::Vector3D(0.4, 4, 1);
  CGL::Vector3D separation = CGL::Vector3D(0.2, 6, 1);
  CGL::Vector3D cohesion = CGL::Vector3D(0.4, 4, 1);
  float wander = 2;
  float boundary = 1;
  float collision = 20;
  float hunt = 10;
  ...
  CGL::Vector3D entities = CGL::Vector3D(200, 0, 0.1);
