...
const string PILLAR = "pillar";

const unordered_set<string> VALID_KEYS = {SPHERE, PLANE, CLOTH, PILLAR};

FishSimulator *app = nullptr;
...
bool loadObjectsFromFile(string filename, Cloth *cloth, ClothParameters *cp, vector<CollisionObject *>* objects, int sphere_num_lat, int sphere_num_lon) {
  ...
  for (json::iterator it = j.begin(); it != j.end(); ++it) {
    ...
    if (key == CLOTH) {
      ...
      auto it_num_fish = object.find("num_fish");
      if (it_num_fish != object.end()) {
        num_fish = *it_num_fish;
      } else {
        incompleteObjectError("cloth", "num_fish");
      }

      auto it_num_predators = object.find("num_predators");
      if (it_num_predators != object.end()) {
        num_predators = *it_num_predators;
      } else {
        incompleteObjectError("cloth", "num_predators");
      }
      ...
      cloth->width = width;
      cloth->height = height;
      cloth->num_fish = num_fish;
      cloth->num_predators = num_predators;
      ...
      auto it_base = object.find("base");
      if (it_base != object.end()) {
        vector<double> vec_base = *it_base;
        base = Vector3D(vec_base[0], vec_base[1], vec_base[2]);
      } else {
        incompleteObjectError("pillar", "base");
      }
      ...
      if (it_radius != object.end()) {
        ...
        incompleteObjectError("pillar", "radius");
      }
      ...
      if (it_height != object.end()) {
        ...
        incompleteObjectError("pillar", "friction");
      }

      Pillar *p = new Pillar(base, radius, height);
      objects->push_back(p);
...
int main(int argc, char **argv) {
  ...
  // Initialize the FishSimulator object
  app = new FishSimulator(project_root, screen);
  app->loadCloth(&cloth);
  ...
  while (!glfwWindowShouldClose(window)) {
    ...
    glClearColor(91.0f / 256, 151.0f / 256, 231.0f / 256, 1.0f);
    ...
    screen->setBackground(nanogui::Color(29, 162, 216, 255));
  }
